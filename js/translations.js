/* Diccionario de traducciones del sitio (ES / EN / PL). */
window.SITE_I18N = {
  es: {
    nav: {
      home: "INICIO",
      about: "SOBRE MÍ",
      book: "BOOK",
      videos: "VIDEOS",
      contact: "CONTACTO"
    },
    common: {
      ver_mas: "Ver más",
      descargar_cv: "Descargar CV",
      cerrar: "Cerrar",
      idiomas: "Idiomas: Español, inglés y polaco",
      habilidades: "Habilidades: Gimnasia rítmica, danza en general",
      conocimientos: "Conocimientos básicos: Lucha escénica y manejo de armas"
    },
    footer: {
      representante: "Representante",
      sigueme: "Sígueme"
    },
    meta: {
      index_title: "LAURA CZARNIK actriz polaca nacida en Madrid",
      index_desc: "Página web de Laura Czarnik, actriz polaca residente en Madrid, con experiencia en publicidad, cortos y videoclips. Conoce sus proyectos, formación, CV, book de fotos, videobook y contacto.",
      about_title: "LAURA CZARNIK ACTRIZ ABOUT ME",
      about_desc: "Página web de Laura Czarnik, actriz polaca residente en Madrid, con experiencia en publicidad, cortos y videoclips. Conoce sus proyectos, formación, CV, book de fotos, videobook y contacto.",
      book_title: "LAURA CZARNIK ACTRIZ BOOK",
      book_desc: "Página web de Laura Czarnik, actriz polaca residente en Madrid, con experiencia en publicidad, cortos y videoclips. Conoce su book actoral.",
      portfolio_title: "LAURA CZARNIK ACTRIZ PORTFOLIO",
      portfolio_desc: "Página web de Laura Czarnik, actriz polaca residente en Madrid, con experiencia en publicidad, cortos y videoclips. Conoce las mejores escenas de Laura Czarnik.",
      contact_title: "LAURA CZARNIK ACTRIZ CONTACTO",
      contact_desc: "Página web de Laura Czarnik, actriz polaca residente en Madrid, con experiencia en publicidad, cortos y videoclips. Contactar con Laura Czarnik."
    },
    contact: {
      breadcrumb_kicker: "CONTACTO",
      title: "Contacto",
      email_placeholder: "Tu correo...",
      message_placeholder: "Tu mensaje...",
      send_btn: "Enviar"
    },
    index: {
      hero_kicker: "ACTRIZ",
      videobook_btn: "VIDEOBOOK",
      about_kicker: "Sobre mí",
      about_bio: `Laura Czarnik es actriz polaco-madrileña, trilingüe en español, inglés y polaco.
                  <br>Su cortometraje "Gracias por nada" ha ganado el Premio del Jurado a Mejor Arte en Corto en el Festival La Raíz, con nominaciones en Festimatge, el Festival de Cortos Rodinia, el FFICAB de Albacete y los IV Premis Costa Brava.
                  <br>Formada en Central de Cine y Work in Progress, combina 8 años de gimnasia rítmica con formación en danza en varios estilos y conocimientos básicos de lucha escénica y manejo de armas.`,
      book_kicker: "BOOK",
      videobook_kicker: "INTERPRETACIÓN"
    },
    about: {
      breadcrumb_kicker: "SOBRE MÍ",
      section_kicker: "Sobre mí",
      bio: `Laura Czarnik es una actriz polaco-madrileña que se mueve con la misma naturalidad entre el drama, la acción y la comedia física. Trilingüe en español, inglés y polaco, su origen europeo la convierte en una pieza poco común en el mapa de casting español: apta tanto para producciones nacionales como para coproducciones con Polonia y el resto de Europa del Este.
             <br>Formada en Central de Cine y Work in Progress, ha protagonizado los videoclips "Tu nene" (Dasoul) y "Boomerang" (Adexe y Nau), y ha participado en cortometrajes como "Dissonantia", "Mirage" y "El Conticinio". Su cortometraje "Gracias por nada" ha sido premiado con el Premio del Jurado a Mejor Arte en Corto en el Festival La Raíz, y ha recibido nominaciones en Festimatge (31º Trofeu Torretes de Curtmetratges), el Festival de Cortos Rodinia, el FFICAB de Albacete y los IV Premis Costa Brava (Mejor Cortometraje y Mejor Guión Original o Adaptado).
             <br>Con 8 años de gimnasia rítmica y formación en danza desde niña en varios estilos, además de conocimientos básicos de lucha escénica y manejo de armas, aprende coreografías y movimiento escénico con rapidez, aportando a cada personaje una base física entrenada.`
    },
    book: {
      filter_todo: "TODO",
      filter_actoral: "ACTORAL",
      filter_habilidades: "HABILIDADES",
      filter_frames: "FRAMES",
      alt_actoral: "Imagen book de la actriz Laura Czarnik",
      alt_habilidades: "Actriz Laura Czarnik haciendo gimnasia rítmica / yoga / ballet",
      alt_frames: "Actriz Laura Czarnik actuando"
    },
    portfolio: {
      videobook_kicker: "INTERPRETACIÓN",
      escenas_kicker: "INTERPRETACIÓN",
      escenas_title: "ESCENAS",
      otros_kicker: "OTROS",
      habilidades_title: "HABILIDADES",
      v_polaco: "Polaco",
      v_thriller: "Thriller",
      v_epoca: "Época",
      v_drama: "Drama",
      v_baile: "Baile",
      v_lucha: "Lucha escénica"
    }
  },

  en: {
    nav: {
      home: "HOME",
      about: "ABOUT ME",
      book: "BOOK",
      videos: "VIDEOS",
      contact: "CONTACT"
    },
    common: {
      ver_mas: "See more",
      descargar_cv: "Download CV",
      cerrar: "Close",
      idiomas: "Languages: Spanish, English and Polish",
      habilidades: "Skills: Rhythmic gymnastics, dance in general",
      conocimientos: "Basic knowledge: Stage combat and weapons handling"
    },
    footer: {
      representante: "Representation",
      sigueme: "Follow me"
    },
    meta: {
      index_title: "LAURA CZARNIK - Polish actress born in Madrid",
      index_desc: "Website of Laura Czarnik, a Polish actress based in Madrid with experience in commercials, short films and music videos. Discover her projects, training, CV, photo book, videobook and contact info.",
      about_title: "LAURA CZARNIK ACTRESS - ABOUT ME",
      about_desc: "Website of Laura Czarnik, a Polish actress based in Madrid with experience in commercials, short films and music videos. Discover her projects, training, CV, photo book, videobook and contact info.",
      book_title: "LAURA CZARNIK ACTRESS - BOOK",
      book_desc: "Website of Laura Czarnik, a Polish actress based in Madrid with experience in commercials, short films and music videos. Discover her acting book.",
      portfolio_title: "LAURA CZARNIK ACTRESS - PORTFOLIO",
      portfolio_desc: "Website of Laura Czarnik, a Polish actress based in Madrid with experience in commercials, short films and music videos. Discover Laura Czarnik's best scenes.",
      contact_title: "LAURA CZARNIK ACTRESS - CONTACT",
      contact_desc: "Website of Laura Czarnik, a Polish actress based in Madrid with experience in commercials, short films and music videos. Get in touch with Laura Czarnik."
    },
    contact: {
      breadcrumb_kicker: "CONTACT",
      title: "Contact",
      email_placeholder: "Your email...",
      message_placeholder: "Your message...",
      send_btn: "Send"
    },
    index: {
      hero_kicker: "ACTRESS",
      videobook_btn: "VIDEOBOOK",
      about_kicker: "About Me",
      about_bio: `Laura Czarnik is a Polish-Madrilenian actress, trilingual in Spanish, English and Polish.
                  <br>Her short film "Gracias por nada" won the Jury Award for Best Art House Short at the Festival La Raíz, with nominations at Festimatge, the Rodinia Short Film Festival, FFICAB Albacete and the 4th Costa Brava Awards.
                  <br>Trained at Central de Cine and Work in Progress, she combines 8 years of rhythmic gymnastics with training in various dance styles and basic knowledge of stage combat and weapons handling.`,
      book_kicker: "BOOK",
      videobook_kicker: "ACTING"
    },
    about: {
      breadcrumb_kicker: "ABOUT ME",
      section_kicker: "About Me",
      bio: `Laura Czarnik is a Polish-Madrilenian actress who moves with equal ease between drama, action and physical comedy. Trilingual in Spanish, English and Polish, her European background makes her a rare fit on the Spanish casting scene: equally suited to national productions and co-productions with Poland and the rest of Eastern Europe.
             <br>Trained at Central de Cine and Work in Progress, she has starred in the music videos "Tu nene" (Dasoul) and "Boomerang" (Adexe y Nau), and has appeared in short films such as "Dissonantia", "Mirage" and "El Conticinio". Her short film "Gracias por nada" won the Jury Award for Best Art House Short at the Festival La Raíz, and has received nominations at Festimatge (31st Trofeu Torretes de Curtmetratges), the Rodinia Short Film Festival, FFICAB Albacete and the 4th Costa Brava Awards (Best Short Film and Best Original or Adapted Screenplay).
             <br>With 8 years of rhythmic gymnastics and dance training since childhood across various styles, plus basic knowledge of stage combat and weapons handling, she picks up choreography and stage movement quickly, bringing a well-trained physicality to every character.`
    },
    book: {
      filter_todo: "ALL",
      filter_actoral: "ACTING",
      filter_habilidades: "SKILLS",
      filter_frames: "FRAMES",
      alt_actoral: "Laura Czarnik's acting book photo",
      alt_habilidades: "Actress Laura Czarnik doing rhythmic gymnastics / yoga / ballet",
      alt_frames: "Actress Laura Czarnik acting"
    },
    portfolio: {
      videobook_kicker: "ACTING",
      escenas_kicker: "ACTING",
      escenas_title: "SCENES",
      otros_kicker: "OTHERS",
      habilidades_title: "SKILLS",
      v_polaco: "Polish",
      v_thriller: "Thriller",
      v_epoca: "Period",
      v_drama: "Drama",
      v_baile: "Dance",
      v_lucha: "Stage combat"
    }
  },

  pl: {
    nav: {
      home: "START",
      about: "O MNIE",
      book: "BOOK",
      videos: "WIDEA",
      contact: "KONTAKT"
    },
    common: {
      ver_mas: "Zobacz więcej",
      descargar_cv: "Pobierz CV",
      cerrar: "Zamknij",
      idiomas: "Języki: hiszpański, angielski i polski",
      habilidades: "Umiejętności: gimnastyka artystyczna, taniec ogólnie",
      conocimientos: "Podstawowa wiedza: walka sceniczna i posługiwanie się bronią"
    },
    footer: {
      representante: "Reprezentacja",
      sigueme: "Obserwuj mnie"
    },
    meta: {
      index_title: "LAURA CZARNIK – polska aktorka urodzona w Madrycie",
      index_desc: "Strona Laury Czarnik, polskiej aktorki mieszkającej w Madrycie, z doświadczeniem w reklamach, krótkich metrażach i teledyskach. Poznaj jej projekty, wykształcenie, CV, book zdjęciowy, videobook i dane kontaktowe.",
      about_title: "LAURA CZARNIK AKTORKA – O MNIE",
      about_desc: "Strona Laury Czarnik, polskiej aktorki mieszkającej w Madrycie, z doświadczeniem w reklamach, krótkich metrażach i teledyskach. Poznaj jej projekty, wykształcenie, CV, book zdjęciowy, videobook i dane kontaktowe.",
      book_title: "LAURA CZARNIK AKTORKA – BOOK",
      book_desc: "Strona Laury Czarnik, polskiej aktorki mieszkającej w Madrycie, z doświadczeniem w reklamach, krótkich metrażach i teledyskach. Poznaj jej book aktorski.",
      portfolio_title: "LAURA CZARNIK AKTORKA – PORTFOLIO",
      portfolio_desc: "Strona Laury Czarnik, polskiej aktorki mieszkającej w Madrycie, z doświadczeniem w reklamach, krótkich metrażach i teledyskach. Poznaj najlepsze sceny Laury Czarnik.",
      contact_title: "LAURA CZARNIK AKTORKA – KONTAKT",
      contact_desc: "Strona Laury Czarnik, polskiej aktorki mieszkającej w Madrycie, z doświadczeniem w reklamach, krótkich metrażach i teledyskach. Skontaktuj się z Laurą Czarnik."
    },
    contact: {
      breadcrumb_kicker: "KONTAKT",
      title: "Kontakt",
      email_placeholder: "Twój e-mail...",
      message_placeholder: "Twoja wiadomość...",
      send_btn: "Wyślij"
    },
    index: {
      hero_kicker: "AKTORKA",
      videobook_btn: "VIDEOBOOK",
      about_kicker: "O mnie",
      about_bio: `Laura Czarnik jest polsko-madrycką aktorką, mówi biegle po hiszpańsku, angielsku i polsku.
                  <br>Jej krótki metraż "Gracias por nada" zdobył Nagrodę Jury dla Najlepszego Krótkiego Filmu Artystycznego na festiwalu La Raíz, z nominacjami na festiwalach Festimatge, Rodinia Short Film Festival, FFICAB Albacete oraz IV edycji Costa Brava Awards.
                  <br>Wykształcona w Central de Cine i Work in Progress, łączy 8 lat gimnastyki artystycznej z nauką tańca w różnych stylach oraz podstawową wiedzą z zakresu walki scenicznej i posługiwania się bronią.`,
      book_kicker: "BOOK",
      videobook_kicker: "AKTORSTWO"
    },
    about: {
      breadcrumb_kicker: "O MNIE",
      section_kicker: "O mnie",
      bio: `Laura Czarnik to polsko-madrycka aktorka, która porusza się z równą swobodą w dramacie, akcji i komedii fizycznej. Mówiąc biegle po hiszpańsku, angielsku i polsku, dzięki swoim europejskim korzeniom jest rzadkością na hiszpańskiej scenie castingowej: pasuje zarówno do produkcji krajowych, jak i koprodukcji z Polską i resztą Europy Wschodniej.
             <br>Wykształcona w Central de Cine i Work in Progress, zagrała główne role w teledyskach "Tu nene" (Dasoul) i "Boomerang" (Adexe y Nau), a także wystąpiła w krótkometrażówkach takich jak "Dissonantia", "Mirage" i "El Conticinio". Jej krótki metraż "Gracias por nada" zdobył Nagrodę Jury dla Najlepszego Krótkiego Filmu Artystycznego na festiwalu La Raíz, a także nominacje na festiwalu Festimatge (31. edycja Trofeu Torretes de Curtmetratges), Rodinia Short Film Festival, FFICAB Albacete oraz IV edycji Costa Brava Awards (Najlepszy Krótki Metraż i Najlepszy Scenariusz Oryginalny lub Adaptowany).
             <br>Dzięki 8 latom gimnastyki artystycznej i nauce tańca w różnych stylach od dziecka, a także podstawowej wiedzy z zakresu walki scenicznej i posługiwania się bronią, szybko przyswaja choreografie i ruch sceniczny, wnosząc do każdej postaci wytrenowaną fizyczność.`
    },
    book: {
      filter_todo: "WSZYSTKO",
      filter_actoral: "AKTORSKIE",
      filter_habilidades: "UMIEJĘTNOŚCI",
      filter_frames: "KADRY",
      alt_actoral: "Zdjęcie z booka aktorskiego Laury Czarnik",
      alt_habilidades: "Aktorka Laura Czarnik podczas gimnastyki artystycznej / jogi / baletu",
      alt_frames: "Aktorka Laura Czarnik podczas gry aktorskiej"
    },
    portfolio: {
      videobook_kicker: "AKTORSTWO",
      escenas_kicker: "AKTORSTWO",
      escenas_title: "SCENY",
      otros_kicker: "INNE",
      habilidades_title: "UMIEJĘTNOŚCI",
      v_polaco: "Polski",
      v_thriller: "Thriller",
      v_epoca: "Kostiumowy",
      v_drama: "Dramat",
      v_baile: "Taniec",
      v_lucha: "Walka sceniczna"
    }
  }
};
